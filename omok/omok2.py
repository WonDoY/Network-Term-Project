...
def game(request, user):
    ...
    while running :
        ...
        if(player==0):
            while(error):
                try:
                    ...
                    row=int(request.GET['r'])
                    col=int(request.GET['c'])
                    '''
                        입력을 받음 request에서 넘어옴.
                        <div class="row">
                            <input type="text" name="r" placeholder="input row" maxlength=2>
                        </div>
                        <div class="col">
                            <input type="text" name="c" placeholder="input col" maxlength=2>
                        </div>
                    '''                    
                    ...
                    if(isSize(row) and isSize(col) and isEmpty(row,col)):
                        if(checkRule(player,row-1,col-1)):
                            ...
                            SYS="PLAYER 1 RULE BREAK"
                    else: 
                        SYS="Already"


                except:
                    SYS="PLAYER 1 INPUT ERROR"
                if(error):
                    context = {'msg':SYS,'locate':locate}
        ...
        # if Player is win,
        if(Winner): 
            SYS="PLAYER "+str(player)+" IS WINNER"
            running=False
